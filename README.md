...
GUI アプリ(Homebrew cask)・macOS システム設定(Dock/Finder/キーボード等)を
再現する。
...
├── Makefile                  # make switch / build / update / fmt / adopt
├── hosts/
│   └── private/              # ホスト固有設定(個人 PC)
│       ├── default.nix       #   入口: Homebrew cask/brew 等
│       └── macos-defaults.nix #   macOS システム設定(Dock/Finder/キーボード)
├── home/
...
│       ├── cli/  utils.nix
│       └── dev/  languages.nix
└── templates/                # プロジェクト用の devShell 雛形(コピーして使う)
    └── node/ python/ ruby/ go/
```
...
言語ランタイムは **グローバルに node/python/go のみ** を置く(`home/modules/dev/languages.nix`)。
これは「プロジェクト外で雑に動かす土台」で、プロジェクトごとの版固定や ruby などは
**direnv + nix-direnv** で各リポジトリに閉じ込める。

`templates/` に言語別の雛形(`.envrc` + `flake.nix`)を用意してある。使い方:

```sh
# 例: あるプロジェクトで node 環境を閉じる
cp ~/Developer/dotfiles/dotfiles/templates/node/{.envrc,flake.nix} .
direnv allow      # 初回だけ許可。以後 cd で自動 in/out
```

- `.envrc` は `use flake` の 1 行。`cd` で入ると `flake.nix` の `devShells.default` が PATH に乗る。
- 版を固定したいときは `flake.nix` の `nodejs` を `nodejs_22` のように変える(コミットハッシュ不要)。
- 初回 `direnv allow` 時に各プロジェクトの `flake.lock` が生成され、そこで版が固定される。
- ruby テンプレートは gem を `vendor/bundle` に閉じ込める(グローバルを汚さない)。
